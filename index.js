...
const session = require("express-session"); //to create session to store information
...
// post :pour inserer les choses dans la BDD
//put : pour inserer dans la bdd
// '/deplacer ' : creer une route pour deplacer un switch (localhost:3001/ajouter )
// req: pour envoyer les requetes et
//"rep" :pour recupere les resultats
...
app.post("/api/AddSwitch", (req, rep) => {
  // importer les variables du front vers le backend
  ...
  //const Etat_Switch = req.body.Etat_Switch;
      ...
      if (err) {
        ...
        while (Nombre_de_ports_FE > i) {
          db.query(
            ...
            (err, res) => {
              if (err) {
                console.log(err); // afficher le message d'erreur
              } else {
...
//Pour insérer les données dans la table de la base de donnée
/*
app.get("/", (req ,res)=> {
    const sqlInsert = 
    "INSERT INTO tab_switchs   Values ('switch5' , 'S15'  , 'Cisco' , 'modele' , '40503', '708004012' , 'A7' , '2022-04-29' , '3' , '4' , '7', '8' , 'utilisé' )" ;
...
//SELECT VLAN, COUNT(N_port) as nbr_ports FROM tab_ports GROUP BY VLAN
/***le nombres de ports pour chaque VLAN */
app.get("/api/nombre_ports", (req, rep) => {
  // const NomVLAN=req.body.NomVLAN;
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        console.log(err); // afficher le message d'erreur
      } else rep.send(result); // bien fait
    }
  );
});

/*************le nombre de ports selon l'état de port ***************/
/***hadi chart a3mida bayaniya  */
...
app.put("/api/getSwitchRechPrise", (req, res) => {
  ...
  //console.log(" rechesalle ",req.body);
...
//
//backend route qui prend req res comme parametre
//la fonction d'ajout d'un utilisateur
//
//backend route qui prend req res comme parametre
//la fonction d'ajout d'un utilisateur
    ...
    (err, result) => {
      ...
      if (err) { res.send(err);}
        ...
        var mail = nodemailer.createTransport({
          service: 'gmail',
          auth: {
              user: 'suiviswitch@gmail.com', // Your email id
              pass: 'suivideswitch' // Your password
          }
      });
      ...
      mail.sendMail(mailOptions, function(error, info) {
          if (error) { //si le mai n'est pas envoyé
              console.log(1);
    ...
    db.query('SELECT email FROM accounts WHERE username = ? ',
        ...
        (err, result) => {
          //  console.log("result");
            ...
            else {  //ya pas d'erreur avec la requete 
                if(result[0]){
...
// recuperer tous les types des ports 
...
app.get("/CHERCHER_SWITCHES_PAR_Etat/:id", (req, rep) => {
  ...
  if (id !== "Etat") {
    db.query(
      ...
      id, // je peux faire nom_vlan=? or IP=?...
      (err, result) => {
        if (err) {
          console.log(err); // afficher le message d'erreur
        } else rep.send(result); // bien fait
...
app.get("/CHERCHER_SWITCHES_PAR_bloc", (req, rep) => {
  ...
  db.query(
    ...
    [bloc], // je peux faire nom_vlan=? or IP=?...
    (err, result) => {
      if (err) {
        console.log(err); // afficher le message d'erreur
      } else rep.send(result); // bien fait
...
// recuperer les ports selon le filtre 
// (SwitchPort=? and Etat_Port =? )
...
/**Define the port where it will work  */
app.listen(5000, () => {
